/*******************************************************************************
* write_ascii.c: this file is part of the EZmock program.
...
int output_writeline(OFILE *ofile, const char *restrict format, ...) {
  ...
  if (ofile->size == ofile->max) {
    if (output_flush(ofile)) return EZMOCK_ERR_FILE;
  }
  ...
  if (size < 0) {
    ...
    return EZMOCK_ERR_ASCII;
  }
  if (size >= EZMOCK_MAX_CHUNK) {
  ...
  if (size + ofile->size >= ofile->max) {
    ...
    while (size >= ofile->max) {
      if (EZMOCK_MAX_CHUNK / 2 < ofile->max) ofile->max = size + 1;
      else ofile->max <<= 1;
      enlarge = true;
    }
